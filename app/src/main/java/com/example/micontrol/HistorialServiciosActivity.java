...
import com.example.micontrol.adaptadores.ListaServiciosAdapter;
...
public class HistorialServiciosActivity extends AppCompatActivity implements SearchView.OnQueryTextListener {

    RecyclerView listaServicios;
    ArrayList<Servicios> listaArrayServicios;
    SearchView txtBuscar;
    ListaServiciosAdapter adapter;
    TextView txtNoServicios;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        txtNoServicios = findViewById(R.id.txtNoServicios);
        listaServicios.setLayoutManager(new LinearLayoutManager(this));

        DbServicios dbServicios = new DbServicios(HistorialServiciosActivity.this);

        listaArrayServicios = new ArrayList<>();

        adapter = new ListaServiciosAdapter(dbServicios.mostrarHistorialServicios());
        listaServicios.setAdapter(adapter);

        // Verificar si no hay citas y mostrar el mensaje correspondiente
        if (adapter.getItemCount() == 0) {
            txtNoServicios.setVisibility(View.VISIBLE);
        } else {
            txtNoServicios.setVisibility(View.GONE);
        }
        ...
        applyBackground(findViewById(R.id.hist_s), getStartColor(AppPreferences.getSelectedColor(this)), getCenterColor(AppPreferences.getSelectedColor(this)), getEndColor(AppPreferences.getSelectedColor(this)));
        ...
        bottomNavigationView.setSelectedItemId(R.id.historial_servicios);
